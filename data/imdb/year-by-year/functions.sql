...
begin
  ...
  (_year is null or tmy.startYear between 
      _year - floor((_num_years) / 2) 
  and _year + _num_years - 1 - floor((_num_years) / 2))

  and ( _title_type is null or tmy.titleType = _title_type )
  and ( _has_cover is null or tmy.cover_url is not null ) 

  and (
      _genres is null or _genres = ''
      or string_to_array(tmy.genres, ',')  && string_to_array(_genres, ',')
      -- or position(_genre in tmy.genres) > 0 
  )
  order by 
    array_intersect_count(
        string_to_array(tmy.genres, ','),
  ...
  select * 
  from movies
  where movies.local_rank between 1 and _max_local_rank
  order by rank
limit 1000
  ;
end$$
...
\x
select *
from get_movies(2018, '', null, null,2, 5)
limit 10;